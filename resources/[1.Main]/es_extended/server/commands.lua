...
ESX.RegisterCommand('setfaction', 'admin', function(xPlayer, args, showError)
	if not ESX.DoesFactionExist(args.faction, args.grade) then
		return showError(TranslateCap('command_setfaction_invalid'))
	end

	args.playerId.setFaction(args.faction, args.grade)
	if Config.AdminLogging then
		ESX.DiscordLogFields("UserActions", "Set Faction /setfaction Triggered!", "pink", {
			{ name = "Player",  value = xPlayer and xPlayer.name or "Server Console",   inline = true },
			{ name = "ID",      value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
			{ name = "Target", value = args.playerId.name, inline = true },
	...
	help = TranslateCap('command_setfaction'),
	validate = true,
	arguments = {
		{ name = 'playerId', help = TranslateCap('commandgeneric_playerid'), type = 'player' },
...
ESX.RegisterCommand("refreshfactions", 'admin', function()
	ESX.RefreshFactions()
end, true, { help = TranslateCap('command_clearall') })
...
ESX.RegisterCommand('faction', { "user", "admin" }, function(xPlayer, _, _)
	print(('%s, your faction is: ^5%s^0 - ^5%s^0'):format(xPlayer.getName(), xPlayer.getFaction().name,
		xPlayer.getFaction().grade_label))
end, false)

ESX.RegisterCommand('info', { "user", "admin" }, function(xPlayer)
	local job = xPlayer.getJob().name
	local faction = xPlayer.getFaction().name
	print(('^2ID: ^5%s^0 | ^2Name: ^5%s^0 | ^2Group: ^5%s^0 | ^2Job: ^5%s^0 | ^2Faction: ^5%s^0'):format(xPlayer.source, xPlayer.getName(),
		xPlayer.getGroup(), job, faction))
end, false)
